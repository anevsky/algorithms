...
public class BoggleTrieST<Value> {
  ...
  private static final int R_START_INDEX = 'A';

  private final Map<String, Queue<String>> prefixMap;
  ...
  private Node get(Node x, String key, int d) {
    ...
    int c = key.charAt(d) - R_START_INDEX;
    return get(x.next[c], key, d + 1);
  }

  /**
   * Inserts the key-value pair into the symbol table, overwriting the old value with the new value
   * if the key is already in the symbol table. If the value is {@code null}, this effectively deletes
   ...
   * in the symbol table named {@code st}, use the foreach notation: {@code for (Key key : st.keys())}.
   *
  ...
  public Iterable<String> keysWithPrefix(String prefix) {
    final Queue<String> queue = prefixMap.get(prefix);
